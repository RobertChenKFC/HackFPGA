...
void Bootstrap(std::iostream &outputFile, bool init, Word &returnNum) {
  // The memory range 16 ~ 255 (inclusive) is the statics range, which is used
  // to store the static variables. Some programs rely on the fact that all
  // static variables are initialized to zero, so this portion of assembly
  ...
  outputFile
    ...
    << "@SP" << std::endl
    << "M=D" << std::endl
    << "(TRANSLATOR_clearLoop)" << std::endl
    << "@SP" << std::endl
    << "D=M" << std::endl
    << "@256" << std::endl
    << "D=D-A" << std::endl
    << "@TRANSLATOR_clearEnd" << std::endl
    << "D;JEQ" << std::endl
    << "@SP" << std::endl
    << "A=M" << std::endl
    << "M=0" << std::endl
