...
extern "C" {
  ...
  void Assemble() {
    auto machineCodeString = Assembler::assemble(assemblyString);
    int idx = 0;
    for (size_t i = 0; i < machineCodeString.length(); i += 33) {
      program[idx] = 0;
      for (size_t j = 0; j < 32; ++j) {
        program[idx] |=
            static_cast<Word>(machineCodeString[i + j] - '0') << (31 - j);
      }
      ++idx;
    }

    EM_ASM(console.log('Assembled program');, 0);
  }
